...
def build_pace_features(
    ...
    Espera receber `laps` já filtrado — ver `select_green_flag_laps`
    (`data/laps.py`). Não filtra de novo aqui, pra manter a função pura e
    testável com qualquer subconjunto controlado nos testes, sem acoplar
    às duas regras de negócio ao mesmo tempo.
